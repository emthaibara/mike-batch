...
def __store_tasks_job():
    ...
    if all(task[1] == StatusEnum.completed.value for task in cached_tasks.items()):
        global __is_completed
        __is_completed = True
        __logger.debug('all task completed !')
    else:
        persistence(False)
        # __logger.debug('定时同步所有模拟任务状态 cache tasks -------> tasks.json store successfully !')
