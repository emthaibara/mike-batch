...
__start_time = '2022-01-01 08:00:00' #起始时间
__time_step_seconds = 10 # 秒
__preheat_time = 2160 # 0.6h x 3600s = 2160s
__number_of_preheat_steps = 216 # 2160 / 10 = 216
def gen_dfs0(duration_hour : int,
             flow_rate: float,
             dfs0_type: str,     # 'Qlhk'---q1 'Qyg'----q3 'Qcs'----q2
             write_path: str):
    time = duration_hour * 3600
    number_of_time_steps = int(time / __time_step_seconds)

    # 预热偏移量（流量匀速增长至flow_rate） 如果是Qcs偏移量设置为0
    offset = flow_rate / __number_of_preheat_steps if dfs0_type != 'Qcs' else 0
    # 时间序列
    time_list = [pd.Timestamp(__start_time) + pd.Timedelta(seconds=i * __time_step_seconds) for i in range(__number_of_preheat_steps + number_of_time_steps)]
    # 准备数据
    # 预热部分
